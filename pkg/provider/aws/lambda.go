...
func (awsPrv *AWSProvider) FetchFunctions(ctx context.Context, output chan<- types.FunctionConfiguration) error {
	...
	for paginator.HasMorePages() {
		...
		if err := util.SendAllFromSlice(ctx, output, result.Functions); err != nil {
			return err
		}
	}
